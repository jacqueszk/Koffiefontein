...
Predictive insights and variation of components influencing endogenous variables give on-mine decision maker a better chance of control based on historically observed data. The aim of the projects were to fit supervised regression/classification models on the applicable, delineated, exogenous variables. After the optimal models were selected, the prediction algorithms were implemented into production. The endogenous predictions and the exogenous variables were then displayed in the control room and on management dashboards for improved process control. 
...
* Push underperfroming exogenous variables to screens for process control.
...
**Emerging field: predicitve to prescriptive**
...
All the above mentioned pillars can be modelled as endogenous variables.

The first goal will be to collect or generate all possible exogenous variables dogenous variables


The model that has been elicited makes it possible to predict on hourly data. It will be put into production using SQLMachine learning services. The current hour's prediction will be based on the real-time quantities of the exogenous variables. 
...
**Grade prediction**
Build model and predict (data at tune level will be required at least)
