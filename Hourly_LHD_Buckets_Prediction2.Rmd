---
title: "Koffiefontein Machine Learning Pilot Project"
output:
  html_document:
      # toc: true
           df_print: paged
...
Hourly data for February and March 2019 were used for the modelling and predictions. The plan going forward is to automate the inclusion of all applicable hourly data into the ever-growing training dataset as they occur. Temporal data inclusion is imperative as it will be a key independent variable representing the change in the minig block over time. The different models will than automatically retrain on a weekly basis and potentially improve their prediction capability.The ultimate aim of the projects are to move from the predictive to prescriptive space.
...
Hourly data for February and March 2019:
...
```{r echo=TRUE}
# Read train data and inspect (February 2019)
...
ggplot(Specific_day, aes(x = Instant, y = value, color = valuetype, linetype = valuetype)) + 
  ...
  scale_color_brewer(palette = "Dark2") + 
  ggtitle("Predicted 10 March 2019 Buckets, Random Forest plot")
...
The model that has been inferred makes it possible to predict on hourly data. It will be put into production using SQLMachine learning services. The current hour's prediction will be based on the real-time quantities of the independent variables. 
