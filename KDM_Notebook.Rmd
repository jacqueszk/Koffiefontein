...
* (56 level ring meters drilled past month: inclusion increased RMSE)
* (58 level ring meters drilled past month: inclusion increased RMSE)
* (60 level ring meters drilled past month: inclusion increased RMSE)
...
set.seed(1)
(outcome <- "Buckets")
(vars <- c("Weekday", "Hour", "Operators", "LHDs","FractionFrom56.W","FractionFrom58.E","FractionFrom58.R","FractionFrom60.E","FractionFrom60.W","DistinctTunnels56.W",  "DistinctTunnels58.E","DistinctTunnels58.R","DistinctTunnels60.E","DistinctTunnels60.W"))
...
nrounds = 19,       # number of trees to build
...
Specific_day <- KDMTestCSV %>% 
  ...
  gather(key = valuetype, value = value, Buckets, pred2) %>%
  # Filter for rows 
  filter(Instant <= 9*24 & Instant > 8*24)
...
ggplot(Specific_day, aes(x = Instant, y = value, color = valuetype, linetype = valuetype)) + 
  ...
  scale_color_brewer(palette = "Dark2") + 
  ggtitle("Predicted 9 March 2020 Buckets, Random Forest plot")
